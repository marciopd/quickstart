...
public class Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response ) {
		...
		try {
            ...
            try {
                AppOne proxy = (AppOne)lookup(response, iCtx, "ejb:appone/ejb//AppOneBean!org.jboss.as.quickstarts.ejb.multi.server.app.AppOne");
                if(proxy != null) {
            ...
            try {
                AppTwo proxy = (AppTwo)lookup(response, iCtx, "ejb:apptwo/ejb//AppTwoBean!org.jboss.as.quickstarts.ejb.multi.server.app.AppTwo");
                if(proxy != null) {
                    write(response, "Invocation #1 return node.name => "+ proxy.getJBossNodeName()+"<br/>");
                    // second invocation shows whether the same or a different node is reached
                    write(response, "Invocation #2 return node.name => "+ proxy.getJBossNodeName()+"<br/>");
		...
		if(fail) {
		    write(response,"<br/><br/><br/><p><b><i>Not all invocations are successful, see JBOSS_HOME/domain/servers/app-web/log/server.log</i></b></p>");
		}else{
